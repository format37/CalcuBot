...
<?php
//updated: 2018.08.10
define('BOT_TOKEN', '**copypasteBotTokenThere**');
...
function checkDangerousWrongQuery($queryToCheck)
{
	...
	for ($i=0;$i<count($dangerousWords);$i++)
	{
		if (substr_count($queryToCheck,$dangerousWords[$i])) return '"[dangerous query '.$dangerousWords[$i].']"';
		if (substr_count($queryToCheck,$wrongWords[$i])) return '"[wrong symbol '.$wrongWords[$i].']"';
	}*/
	return $queryToCheck;
}
...
function checkWords($query)
{
	...
	{
		if (array_search($words[$i],$goodWords)===false) $badWords=$badWords.$words[$i]." ";
	}
	return $badWords;
}
...
function myeval($mycode)
{
		...
		else
		{
			...
			foreach ($queryParts as $part)
			{
				...
				else
				{
					...
					if (strlen($wrongWords)) $calculation.='Wrong words: '.$wrongWords;
					elseif (eval('$calculation='.checkDangerousWrongQuery(str_replace(' ','',strtolower($part))).';')===false) return false;
					$res.=$calculation;
				}
				$isText=!$isText;
			}
	...
	{
		if (eval('$res='.$mycode.";")===false) return false;
		else return $res;
	}
...
{
	$chat	= '-1001114573373';
	file_get_contents('https://api.telegram.org/bot'.BOT_TOKEN.'/sendMessage?chat_id='.$chat.'&text=exit');//DEBUG
	exit;
}
...
if (isset($update["message"]))
{
		...
		$wrongWords	= checkWords($update["message"]['text']);
		if (strlen($wrongWords)) $message='Wrong words: '.$wrongWords;
        else $message	= checkDangerousWrongQuery($update["message"]['text']);
...
if (isset($update["inline_query"])) {
    ...
    $badRequest	= FALSE;
		...
		//if (strpos($source,"sleep")!==false||strpos($source,"while")!==false||strpos($source,"$")!==false)	{addInlineResult("13","please dont hack me",	"wrong query");$badRequest	= TRUE;}
		$result	= myeval($source);
