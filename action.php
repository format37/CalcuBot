...
function requestLog($token,$chat,$clear)
{
	$host='MYSECRET_HOST';
	$database='MYSECRET_SQLDATABASE';
	$user='MYSECRET_SQLUSERNAME';
	$pswd='MYSECRET_SQLPASSWORD';
	...
	if ($clear)
	{
		...
		$AnswerText	= 'log clearing succesfull!';
		file_get_contents('https://api.telegram.org/bot'.$token.'/sendMessage?chat_id='.$chat.'&text='.prepareStringForReturn($AnswerText));
	}
	else {
		$query = "SELECT * FROM `history`";
		//$query = "INSERT INTO `u440813_calcubot`.`history` (`date`, `query`, `user`) VALUES ('".date("Y-m-d H:i:s")."', '".$telegramQuery."', '".$telegramUser."')";
		$res = mysql_query($query);
		$AnswerText	= '';
		$counter=0;
		while($row = mysql_fetch_array($res))
		{
			...
			$AnswerText	= $AnswerText.$row['date'].' // '.$row['user'].' // '.str_replace('%','PERCENT',$row['query']).'%0a';
//$AnswerText	= str_replace('%','PERCENT',$AnswerText);
	$value	=
			$counter++;
			if ($counter>30)
			{
		...
		if ($AnswerText!='') file_get_contents('https://api.telegram.org/bot'.$token.'/sendMessage?chat_id='.$chat.'&text='.prepareStringForReturn($AnswerText));
	}
...
function saveToLog($telegramQuery,$telegramUser)
{
	$host='MYSECRET_HOST';
	$database='MYSECRET_SQLDATABASE';
	$user='MYSECRET_SQLUSERNAME';
...
if ($message=='/requestlogread') requestLog($token,$chat,false);
if ($message=='/requestlogclear') requestLog($token,$chat,true);

if ($message=='/help@calcubot'||$message=='/help'||$message=='/start') 
	{
	$AnswerText	= "Hi! im console calculator рџ‘» based on PHP and his eval function";
...
if ($message=='/about@calcubot'||$message=='/about') 
	{
	$AnswerText	= "v 1.4 Developer Alexey Yurasov%0aformat37@gmail.com%0a@AlexMoscow";
	file_get_contents('https://api.telegram.org/bot'.$token.'/sendMessage?chat_id='.$chat.'&text='.prepareStringForReturn($AnswerText));
	}
	
$crop=0;
if (substr($message,0,12)=='/cl@calcucot') $crop=12;
...
{
	$badRequest	= true;
	file_get_contents('https://api.telegram.org/bot'.$token.'/sendMessage?chat_id='.$chat.prepareStringForReturn('&text=^ is wrong symbol. Use pow(a,b)'));
}

if (strExists("sleep", $source))
{
	$badRequest	= true;
	file_get_contents('https://api.telegram.org/bot'.$token.'/sendMessage?chat_id='.$chat.'&text=please%20dont%20hack%20me.%20i%20just%20try%20to%20make%20world%20better)');
}
